...
class Theme_My_Login_User_Moderation extends Theme_My_Login_Module {
	...
	function apply_user_activation_notification_filters() {
		if ( $GLOBALS['theme_my_login']->is_module_active( 'custom-email/custom-email.php' ) && $options = $GLOBALS['theme_my_login']->options->get_option( array( 'email', 'user_activation' ) ) ) {
			$GLOBALS['theme_my_login_custom_email']->set_mail_headers( $options['mail_from'], $options['mail_from_name'], $options['mail_content_type'] );
	...
	function apply_user_approval_notification_filters() {
		if ( $GLOBALS['theme_my_login']->is_module_active( 'custom-email/custom-email.php' ) && $options = $GLOBALS['theme_my_login']->options->get_option( array( 'email', 'user_approval' ) ) ) {
			$GLOBALS['theme_my_login_custom_email']->set_mail_headers( $options['mail_from'], $options['mail_from_name'], $options['mail_content_type'] );
	...
	function apply_user_approval_admin_notification_filters() {
		if ( $GLOBALS['theme_my_login']->is_module_active( 'custom-email/custom-email.php' ) && $options = $GLOBALS['theme_my_login']->options->get_option( array( 'email', 'user_approval' ) ) ) {
			$GLOBALS['theme_my_login_custom_email']->set_mail_headers( $options['admin_mail_from'], $options['admin_mail_from_name'], $options['admin_mail_content_type'] );
	...
	function apply_user_denial_notification_filters() {
		if ( $GLOBALS['theme_my_login']->is_module_active( 'custom-email/custom-email.php' ) && $options = $GLOBALS['theme_my_login']->options->get_option( array( 'email', 'user_denial' ) ) ) {
			$GLOBALS['theme_my_login_custom_email']->set_mail_headers( $options['mail_from'], $options['mail_from_name'], $options['mail_content_type'] );
	...
	function user_activation_notification_title_filter( $title, $user_id ) {
		...
		return empty( $_title ) ? $title : $GLOBALS['theme_my_login_custom_email']->replace_vars( $_title, $user_id );
	}
	...
	function user_activation_notification_message_filter( $message, $activation_url, $user_id ) {
		...
		return empty( $_message ) ? $message : $GLOBALS['theme_my_login_custom_email']->replace_vars( $_message, $user_id, array( '%activateurl%' => $activation_url ) );
	}
	...
	function user_approval_notification_message_filter( $message, $new_pass, $user_id ) {
		...
		return empty( $_message ) ? $message : $GLOBALS['theme_my_login_custom_email']->replace_vars( $_message, $user_id, array( '%loginurl%' => $GLOBALS['theme_my_login']->get_login_page_link(), '%user_pass%' => $new_pass ) );
	}
	...
	function user_approval_admin_notification_message_filter( $message, $user_id ) {
		...
		return empty( $_message ) ? $message : $GLOBALS['theme_my_login_custom_email']->replace_vars( $_message, $user_id, array( '%pendingurl%' => admin_url( 'users.php?role=pending' ) ) );
	}
	...
	function user_denial_notification_message_filter( $message, $user_id ) {
		...
		return empty( $_message ) ? $message : $GLOBALS['theme_my_login_custom_email']->replace_vars( $_message, $user_id );
	}
