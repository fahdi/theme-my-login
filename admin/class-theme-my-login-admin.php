...
class Theme_My_Login_Admin {
	...
	function module_errors() {
		$module_errors = $this->theme_my_login->options->get_option( 'module_errors' );
		...
		if ( $module_errors ) {
			...
			$this->theme_my_login->options->delete_option( 'module_errors' );
	...
	function initial_nag() {
		if ( $this->theme_my_login->options->get_option( 'initial_nag' ) && current_user_can( 'manage_options' ) ) {
	...
	function load_settings_page() {
		...
		if ( current_user_can( 'manage_options' ) ) {
			...
			if ( $this->theme_my_login->options->get_option( 'initial_nag' ) )
				$this->theme_my_login->options->set_option( 'initial_nag', 0, true );
...
<?php
	...
	function display_module_settings() {
		...
		$active_modules = (array) $this->theme_my_login->options->get_option( 'active_modules' );
	?>
<table class="form-table">
    <tr valign="top">
...
<?php
	...
	function save_settings( $settings ) {
		...
		if ( $activate = array_diff( (array) $modules, (array) $options->get_option( 'active_modules' ) ) ) {
			...
			if ( is_wp_error( $result ) ) {
				...
				foreach ( $result->get_error_data( 'modules_invalid' ) as $module => $wp_error ) {
					...
					if ( is_wp_error( $wp_error ) )
						$options->options['module_errors'][$module] = $wp_error->get_error_message();
				}
			}
		}

		// If we have modules to deactivate
		if ( $deactivate = array_diff( (array) $options->get_option( 'active_modules' ), $modules ) ) {
			// Deactive them
			$this->deactivate_modules( $deactivate );
		}

		// Merge current settings
		$settings = $this->theme_my_login->array_merge_recursive( $options->options, $settings );
	...
	function activate_module( $module ) {
		...
		$current = (array) $this->theme_my_login->options->get_option( 'active_modules' );
		if ( !$this->theme_my_login->is_module_active( $module ) ) {
			...
			$this->theme_my_login->options->set_option( 'active_modules', $current );
